...
\journalname{Information Geometry}

% proposition, corollary, theorem, proof already defined by svjour3.cls
...
\keywords{Fisher information \and Product manifolds \and Phase coupling \and Biological oscillations \and Dimensional coherence}
\end{abstract}
...
Biological systems operate through nested oscillatory hierarchies. In neural systems, gamma oscillations (30--80 Hz) are modulated by theta rhythms (4--8 Hz), which are in turn modulated by slower delta and infra-slow fluctuations \cite{buzsaki2006rhythms}. In cellular biology, circadian rhythms (24 h) organize ultradian metabolic cycles (hours) which contain faster enzymatic oscillations (seconds to minutes) \cite{lloyd2006ultradian}. In ecosystems, seasonal cycles modulate shorter reproductive and behavioral rhythms \cite{helm2017annual}.

This hierarchical frequency organization is not incidental. Recent theoretical work proposes that intelligence arises from the maintenance of high-dimensional coherent dynamics \cite{todd2026intelligence}---systems that sustain many coupled degrees of freedom beyond what external observers can track. But where does this dimensionality come from?
...
\textbf{Main results.} We establish three theorems:
\begin{enumerate}
    \item \textbf{Product structure} (Theorem \ref{thm:product}): $K$ nested frequency bands create a phase manifold $\mathcal{M} \cong T^K / S^1 \cong T^{K-1}$, a $(K-1)$-dimensional torus after quotienting global phase.
...
\textbf{Relation to prior work.} The information geometry of oscillator systems has been studied extensively \cite{amari2016information,nakahara2002information}. Our contribution is identifying the role of \textit{frequency nesting} (not merely multiple oscillators, but hierarchical phase-amplitude coupling) and \textit{clock constraints} (the collapse induced by global synchronization). This connects information geometry to the emerging theory of dimensional constraints on intelligence \cite{todd2026intelligence}.
...
A \textit{statistical manifold} $\mathcal{M}$ is a smooth family of probability distributions $\{p(x|\theta) : \theta \in \Theta\}$ equipped with the Fisher information metric:
\begin{equation}
g_{ij}(\theta) = \mathbb{E}\left[\frac{\partial \log p}{\partial \theta^i} \frac{\partial \log p}{\partial \theta^j}\right]
\end{equation}
The \textit{Fisher rank} at $\theta$ is $\mathrm{rank}(g(\theta))$: the number of statistically distinguishable directions. Parameters in the kernel of $g$ cannot be identified from observations.

\subsection{Phase variables and the torus}

A system with $K$ oscillators has phase variables $\phi = (\phi_1, \ldots, \phi_K) \in T^K$, where $T^K = (S^1)^K$ is the $K$-torus. The global phase $\Phi = \frac{1}{K}\sum_i \phi_i$ is often unobservable (only relative phases matter). Quotienting by global phase gives the \textit{reduced phase manifold}:
\begin{equation}
\mathcal{M}_{\text{phase}} = T^K / S^1 \cong T^{K-1}
\end{equation}
with coordinates $\psi_j = \phi_j - \phi_1$ for $j = 2, \ldots, K$.

\subsection{Nested frequency hierarchies}

A \textit{nested frequency hierarchy} consists of $K$ oscillators with frequencies $\omega_1 < \omega_2 < \cdots < \omega_K$ (from slow to fast) and \textit{phase-amplitude coupling}: the phase of slower oscillations modulates the amplitude or instantaneous frequency of faster oscillations.

The canonical example is theta-gamma coupling in hippocampus \cite{lisman2013theta}: gamma bursts occur preferentially at specific phases of the theta cycle. The gamma \textit{phase} carries information; the theta \textit{phase} determines when that information is relevant.
...
\textbf{Example: Neural oscillations.} Consider four nested bands: delta (0.5--4 Hz), theta (4--8 Hz), gamma (30--80 Hz), and high gamma (80--150 Hz). This creates a 3-dimensional phase manifold. The neural state can be specified by three relative phases, each carrying independent information.
...
The product structure establishes the \textit{ambient dimension}. But dimension alone does not determine identifiability. What matters is the Fisher rank: how many directions are statistically distinguishable.
...
\label{thm:coupling}
Let $p_0(\psi)$ be the product distribution on $T^{K-1}$ with Fisher matrix $g_0 = \mathrm{diag}(g_1, \ldots, g_{K-1})$. Let $p_\kappa(\psi) \propto p_0(\psi) \exp(\kappa \cdot C(\psi))$ be a coupling perturbation, where $C: T^{K-1} \to \mathbb{R}$ depends on multiple relative phases (e.g., $C(\psi) = \cos(\psi_j - \psi_k)$ for phase-phase coupling).
...
A system of $K$ oscillators is \textit{clock-synchronized} if all phases are constrained to a common reference:
\begin{equation}
...
Under clock synchronization, the relative phases are \textit{fixed}:
\begin{equation}
...
\end{enumerate}
Neuromorphic computing architectures that incorporate these features \cite{indiveri2011neuromorphic} may approach biological Fisher rank in ways that clocked digital systems cannot.
...
Buzs\'aki, G. (2006). \textit{Rhythms of the Brain}. Oxford University Press.

\bibitem{helm2017annual}
Helm, B., et al. (2017). Annual rhythms that underlie phenology: biological time-keeping meets environmental change. \textit{Proceedings of the Royal Society B}, 284, 20170046.
...
Lisman, J.~E., Jensen, O. (2013). The theta-gamma neural code. \textit{Neuron}, 77, 1002--1016.

\bibitem{lloyd2006ultradian}
Lloyd, D., Rossi, E.~L. (2006). \textit{Ultradian Rhythms from Molecules to Mind}. Springer.

\bibitem{nakahara2002information}
Nakahara, H., Amari, S.-I. (2002). Information-geometric measure for neural spikes. \textit{Neural Computation}, 14, 2269--2316.

\bibitem{todd2026intelligence}
Todd, I. (2026). Intelligence as high-dimensional coherence: The observable dimensionality bound and computational tractability. \textit{BioSystems}, 258, 105704.
